...
class TestRiskAsset:
    @allure.feature("安全总览")
    @pytest.mark.parametrize(argnames="cases", argvalues=HandleExcle(file_path() + "/data/SecurityOverview.xlsx", "riskasset").read_data())
    def test_riskasste(self, cases, start_up):
        casename, row, url, method, headers, request_type, data, expected, \
        redis_client, ck_client, risk_table, sendrequest = start_up
        allure.dynamic.story("资产风险")
        # 动态设置报告中的用例名称
