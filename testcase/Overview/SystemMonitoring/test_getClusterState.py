...
class TestGetClusterState:
    ...
    @pytest.mark.parametrize(argnames="cases", argvalues=HandleExcle(file_path() + "/data/SystemMonitoring.xlsx", "getClusterState").read_data())
    def test_getClusterState(self, cases, start_up):
        ...
        with allure.step("结果回写到临时变量"):
            ...
            if jsonp != None:
                flinkCluster_expr = HandleJsonPath.jsonpath_split(jsonpathstr=jsonp, variable="flinkCluster")
                flinkCluster = jsonpath(res, flinkCluster_expr)
                data = {"flinkCluster": flinkCluster}
                yaml_util.write_yaml(data=data)
                logging.info("flinkCluster的值为：{}".format(flinkCluster))
