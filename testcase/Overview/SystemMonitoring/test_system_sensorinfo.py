...
@allure.epic("总览")
class TestSystemSensorInfo:
    # 通过依赖的方式一直被跳过
    # @pytest.mark.dependency(depends=['SystemMonitoring/test_systemsensors.py::TestSystemSensors::test_systemsensors'], scope='session')
    @pytest.mark.run(order=-1)
    @pytest.mark.parametrize(argnames="cases", argvalues=HandleExcle(file_path() + "/data/SystemMonitoring.xlsx", "sensorinfo").read_data())
    def test_systemsensorinfo(self, cases, start_up):
