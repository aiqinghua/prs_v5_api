...
@allure.story("风险趋势图")
class TestRiskTrend:
    excle = HandleExcle(file_path() + "/data/SecurityOverview.xlsx", "riskTrend")
    ...
    def test_risktrend(self, cases):
        ...
        if data != None:
            ...
            """
            # if quert_day == 7:
            #     data = data.replace("${start_time}", str(self.risk_time.sevenday_zero0()))
            #     data = data.replace("${end_time}", str(self.risk_time.new_time()))
            # elif quert_day == 3:
            #     data = data.replace("${start_time}", str(self.risk_time.threeday_zero0()))
            #     data = data.replace("${end_time}", str(self.risk_time.new_time()))
            # elif quert_day == 1:
            #     data = data.replace("${start_time}", str(self.risk_time.newday_zero0()))
            #     data = data.replace("${end_time}", str(self.risk_time.new_time()))
